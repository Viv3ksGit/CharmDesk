...
- **Right-click** the charm, or use the **system tray icon** (Windows) /
  **menu bar icon** (macOS), to access: Show/Hide, Perform ritual, toggle
  petal animation, resize, change the charm image, check for updates, Start
  with Windows/at Login, and Quit.
- **Global shortcuts**: `Ctrl+Shift+D` toggles visibility, `Ctrl+Shift+R`
  performs the ritual, `Ctrl+Shift+=` / `Ctrl+Shift+-` resize the charm.

## Updates

The installed app checks GitHub Releases for a newer version on launch and
every 4 hours, downloads it automatically, and prompts to restart once it's
ready. This only runs in the packaged/installed app, not via `npm start`.
Windows updates apply automatically; on macOS, auto-update generally needs a
notarized build (this project currently ships an ad-hoc-signed build, so
Mac users may need to reinstall manually until notarization is set up).
